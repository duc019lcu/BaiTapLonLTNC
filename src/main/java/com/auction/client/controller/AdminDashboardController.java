            ...
            Platform.runLater(() -> {
                ...
                for (int i = 1; i < entries.length; i++) {
                    ...
                    if ("RUNNING".equals(status) || "EXTENDED".equals(status))
                        running++;
                    if ("FINISHED".equals(status) || "SUCCESS".equals(status) || "FAILED".equals(status)
                            || "CANCELED".equals(status) || "PAID".equals(status))
                        finished++;
        ...
        confirm.showAndWait().ifPresent(btn -> {
            if (btn == ButtonType.YES) {
                ...
                if (response != null && (response.startsWith("CLOSE_SESSION_SUCCESS")
                        || response.startsWith("CAP_NHAT"))) {
                    showAlert(Alert.AlertType.INFORMATION, "Thanh cong",
                            "Da dong phien: " + selected.getAuctionId());
                    loadDataAsync();
                } else {
                    String msg = (response != null && response.contains("|"))
                            ? response.split("\\|", 2)[1]
                            : "Khong the dong phien.";
                    showAlert(Alert.AlertType.ERROR, "Loi", msg);
                }
            }
        });
    }
